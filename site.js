...
const notchDemoPhases = [
  {
    phase: 'running-tool',
    status: 'running',
    labelKey: 'notchDemo.runningTool',
    icon: '<polyline points="4 17 10 11 4 5"></polyline><line x1="12" x2="20" y1="19" y2="19"></line>',
  },
  {
    phase: 'editing',
    status: 'running',
    labelKey: 'notchDemo.editing',
    icon: '<path d="m18 5-2.414-2.414A2 2 0 0 0 14.172 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2"></path><path d="M21.378 12.626a1 1 0 0 0-3.004-3.004l-4.01 4.012a2 2 0 0 0-.506.854l-.837 2.87a.5.5 0 0 0 .62.62l2.87-.837a2 2 0 0 0 .854-.506z"></path><path d="M8 18h1"></path>',
  },
  {
    phase: 'verifying',
    status: 'running',
    labelKey: 'notchDemo.verifying',
  ...
  },
  {
    phase: 'needs-you',
    status: 'waiting',
    labelKey: 'notchDemo.needsYou',
    icon: '<circle cx="12" cy="12" r="10"></circle><line x1="12" x2="12" y1="8" y2="12"></line><line x1="12" x2="12.01" y1="16" y2="16"></line>',
  },
  {
    phase: 'completed',
    status: 'completed',
    labelKey: 'notchDemo.completed',
    icon: '<path d="M20 6 9 17l-5-5"></path>',
  },
];
let notchDemoPhaseIndex = 0;
let notchDemoTimer;

function renderNotchDemoPhase(index = notchDemoPhaseIndex) {
  const phase = notchDemoPhases[index] || notchDemoPhases[0];
  document.querySelectorAll('[data-notch-demo]').forEach((demo) => {
    const wing = demo.querySelector('[data-notch-wing]');
    const activity = demo.querySelector('[data-notch-activity]');
    const icon = demo.querySelector('[data-notch-icon]');
    const label = demo.querySelector('[data-notch-phase-label]');
    if (!wing || !activity || !icon || !label) return;

    wing.className = `notch-demo-wing notch-demo-wing--right status-${phase.status}`;
    activity.className = `notch-demo-activity status-${phase.status} phase-${phase.phase}`;
    icon.replaceChildren();
    icon.insertAdjacentHTML('afterbegin', phase.icon);
    label.textContent = message(phase.labelKey);
    label.dataset.i18n = phase.labelKey;
    demo.dataset.notchPhase = phase.phase;

    void wing.offsetWidth;
    wing.classList.add('is-changing');
    window.setTimeout(() => wing.classList.remove('is-changing'), 360);
  });
}

function syncNotchDemoLabels() {
  renderNotchDemoPhase(notchDemoPhaseIndex);
}

function stopNotchDemoCycle() {
  window.clearInterval(notchDemoTimer);
  notchDemoTimer = undefined;
}

function startNotchDemoCycle() {
  if (notchDemoTimer || document.hidden) return;
  notchDemoTimer = window.setInterval(() => {
    notchDemoPhaseIndex = (notchDemoPhaseIndex + 1) % notchDemoPhases.length;
    renderNotchDemoPhase(notchDemoPhaseIndex);
  }, 2400);
}
...
function applyThemePreference(preference, persist = true) {
  ...
  if (persist) {
    try {
      localStorage.setItem('vibemeter-theme', preference);
    } catch (_) {}
  }
}
...
const languageStorageKey = 'vibemeter-language';
...
document.addEventListener('visibilitychange', () => {
  if (reducedHeroMotion || document.hidden) {
    stopNotchDemoCycle();
  } else {
    startNotchDemoCycle();
  }
...
const cloudflareAnalyticsToken = document
  .querySelector('meta[name="cloudflare-web-analytics-token"]')
  ?.getAttribute('content')
  ?.trim();

if (cloudflareAnalyticsToken && !['localhost', '127.0.0.1', '::1'].includes(location.hostname)) {
